...
pub(crate) fn get_disk_id() -> Result<String, HWIDError>{
    ...
    let serial = ser.get(0).ok_or(HWIDError::new("UuidError", "Could not retrieve Uuid"))?.serial_number.clone();
    Ok(serial)
}

#[cfg(target_os="windows")]
pub(crate) fn get_mac_address(device:Option<String>) -> Result<String, HWIDError>{
    let con = WMIConnection::new(COMLibrary::new().unwrap().into())?;
    let ser:Vec<MACGeneric> = con.raw_query("SELECT MACAddress from Win32_NetworkAdapter")?;
    Ok(ser.get(0)
    .ok_or(HWIDError::new("MACAddress", "Could not retrieve Mac Address"))?
    .MACAddress.clone())
}